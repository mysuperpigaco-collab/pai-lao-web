...
interface Mission {
  ...
  endDate: string;
  myStatus?: string | null; // SUBMITTED | APPROVED | REJECTED | null
}
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 6, marginBottom: 14 }}>
          {missions.map(m => (
            <button key={m.id} onClick={() => setSelected(m)} style={{
              padding: "8px 12px", borderRadius: 10, border: selected?.id === m.id ? "2px solid #f59e0b" : "1.5px solid #fde68a",
        ...
        <div>
          ...
          {done ? (
            ...
            <>
                  ...
                  <div style={{ display: "flex", gap: 6, flexWrap: "wrap", marginTop: 8 }}>
                    {photos.map((p, i) => (
                      <div key={i} style={{ position: "relative" }}>
                        <img src={p} alt="" style={{ width: 60, height: 60, objectFit: "cover", borderRadius: 6, border: "2px solid #fde68a" }} />
              ...
              <div style={{ marginBottom: 10 }}>
                <div style={{ fontSize: 12, fontWeight: 700, color: "#92400e", marginBottom: 6 }}>💬 บรรยาย / ข้อความประกอบ</div>
                <textarea
              ...
              <button onClick={submit} disabled={submitting || photos.length === 0} style={{
                width: "100%", padding: "11px", borderRadius: 10, border: "none",
                background: photos.length === 0 ? "#d1d5db" : "#f59e0b",
                color: photos.length === 0 ? "#9ca3af" : "white",
                fontSize: 14, fontWeight: 800, cursor: submitting || photos.length === 0 ? "not-allowed" : "pointer",
              }}>
