...
export async function GET() {
  try {
    ...
    const [
      ...
      totalBookmarks, totalTripLikes, totalPlaceLikes,
    ] = await Promise.all([
      prisma.user.count(),
    ...
    const totalLikes = totalTripLikes + totalPlaceLikes;

    const [recentUsers, recentTrips, recentReports, topTrips, topPlaces] = await Promise.all([
      ...
      prisma.trip.findMany({
        ...
        take: 5,
        select: {
          id: true, slug: true, title: true, coverUrl: true, isPublished: true, createdAt: true,
      ...
      prisma.report.findMany({
        ...
        take: 5,
        select: {
          id: true, targetType: true, reason: true, status: true, createdAt: true,
      ...
      prisma.place.findMany({
        ...
        take: 10,
        select: {
          id: true, slug: true, title: true, coverUrl: true, category: true, province: true,
    ...
    return NextResponse.json({ message: "เกิดข้อผิดพลาด: " + err.message }, { status: 500 });
  }
}
