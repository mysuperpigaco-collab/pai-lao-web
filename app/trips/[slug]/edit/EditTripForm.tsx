...
type Props = { initialTrip: InitialTrip; slug: string };

export default function EditTripForm({ initialTrip, slug }: Props) {
  ...
  const [isDraft,         setIsDraft        ] = useState(initialTrip.isDraft ?? false);
  const [isUnsubmitted,   setIsUnsubmitted  ] = useState(
    (initialTrip.isDraft ?? false) || (!initialTrip.isPublished && !initialTrip.approvalStatus)
  );
  ...
  const [coverPreview,     setCoverPreview    ] = useState<string | null>(initialTrip.coverUrl ?? null);
  const [existingCoverUrl, setExistingCoverUrl] = useState(initialTrip.coverUrl ?? "");

  const [galleryFiles,    setGalleryFiles   ] = useState<File[]>([]);
  const [galleryPreviews, setGalleryPreviews] = useState<string[]>([]);
  const [existingGallery, setExistingGallery] = useState<string[]>(initialTrip.gallery ?? []);

  const [title,      setTitle     ] = useState(initialTrip.title ?? "");
  const [content,    setContent   ] = useState(initialTrip.description ?? "");
  const [budget,     setBudget    ] = useState(initialTrip.budget ? String(initialTrip.budget) : "");
  const [mood,       setMood      ] = useState(initialTrip.mood ?? "Cafe Hopping");
  const [tags,       setTags      ] = useState(Array.isArray(initialTrip.tags) ? initialTrip.tags.join(", ") : "");
  const [youtubeUrl, setYoutubeUrl] = useState(initialTrip.youtubeUrl ?? "");
  const [tiktokUrl,  setTiktokUrl ] = useState(initialTrip.tiktokUrl ?? "");
